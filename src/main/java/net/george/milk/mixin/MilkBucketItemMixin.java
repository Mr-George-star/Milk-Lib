...
public abstract class MilkBucketItemMixin extends Item implements FluidModificationItem {
	...
	public boolean placeFluid(@Nullable PlayerEntity player, World world, BlockPos pos, @Nullable BlockHitResult hitResult) {
		...
		boolean canPlace = blockState.canBucketPlace(MilkLib.STILL_MILK);
		boolean bl2 = blockState.isAir() ||
				canPlace ||
				block instanceof FluidFillable fillable && fillable.canFillWithFluid(player, world, pos, blockState, MilkLib.STILL_MILK);
		if (!bl2) {
			...
			fillable.tryFillWithFluid(world, pos, blockState, MilkLib.STILL_MILK.getStill(false));
			this.playEmptyingSound(player, world, pos);
			return true;
		} else {
			if (!world.isClient && canPlace) {
				world.breakBlock(pos, true);
			}

			if (!world.setBlockState(pos, MilkLib.STILL_MILK.getDefaultState().getBlockState(), Block.NOTIFY_ALL | Block.REDRAW_ON_MAIN_THREAD)
